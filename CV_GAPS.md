...
|---|---|---|
| **Internal titles removed everywhere.** "IT Manager — Full Stack Chapter Lead" and "IT Lead" no longer appear on the CV, the homepage, the public markdown, or the chatbot grounding. "IT Manager" reads as internal back-office at a product company. **The tradeoff:** some background checks and HR verifications match on the legal title, so be ready to state it yourself — nothing on the site does it for you now. | `lib/data/profile.ts` (field removed), `RoleItem.tsx`, `ExperienceItem.tsx`, `app/cv/page.tsx` | Re-add an `internalTitle` field to `Role` and render it in `RoleItem` |
